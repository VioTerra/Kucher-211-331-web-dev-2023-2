...
# Поэтому в соответствии каждому порту стоит список и первый (нулевой) элемент списка указывает как воспринимать номера VLAN,
# которые идут дальше.
...
for intf, vlans in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith("trunk allowed vlan"):
            if vlans[0] == 'add':
                print(f" {command} {', '.join(vlans).replace('add, ', 'add ')}")
            if vlans[0] == 'only':
                print(f" {command} {', '.join(vlans).replace('only, ', '')}")
            if vlans[0] == 'del':
                print(f" {command} {', '.join(vlans).replace('del, ', 'remove ')}")
        else:
            print(f" {command}")
